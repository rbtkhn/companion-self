#!/usr/bin/env python3
"""
Operational snapshot for bridge \"Since last bridge\" deltas (companion-self template).

Writes users/<id>/daily-handoff/last-bridge-state.json after a successful bridge.
Not Record truth. See docs/skill-work/work-cadence/bridge-packet-contract.md.

Usage:
  python3 scripts/bridge_last_state.py -u demo --write
  python3 scripts/bridge_last_state.py -u demo --print-delta
"""
...
DEFAULT_USER = os.getenv("COMPANION_USER_ID", "demo")
...
def build_delta_lines(prev: dict[str, Any], cur: dict[str, Any]) -> list[str]:
    ...
    if prev.get("graceMarHead") != cur.get("graceMarHead"):
        out.append("Repo HEAD changed since last bridge.")
    ...
    if prev.get("graceMarWorktreeRisk") != cur.get("graceMarWorktreeRisk"):
        out.append(
            f"Worktree risk class changed ({prev.get('graceMarWorktreeRisk')} → {cur.get('graceMarWorktreeRisk')})."
        )
...
def main() -> int:
    ...
    g.add_argument("--write", action="store_true", help="Write last-bridge-state.json")
    g.add_argument("--print-delta", action="store_true", help="Print delta bullets for packet")
    args = ap.parse_args()
    user = args.user.strip()
    repo_root = args.repo_root.resolve()
    handoff_dir = repo_root / "users" / user / "daily-handoff"
