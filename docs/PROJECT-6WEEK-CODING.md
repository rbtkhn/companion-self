...
**Goal:** Student (or operator) submits "we did X"; backend stages a candidate into PENDING-REVIEW. No LLM; rule-based or manual tagging.
...
**Goal:** Student (or operator) sees PENDING-REVIEW list; approves or rejects each; on approve, merge into Record and EVIDENCE.
...
- Student approves one candidate; EVIDENCE.md and SELF.md (one section) update; candidate removed from queue. Reject removes from queue only.
...
| 6.2 | **Optional seed flow:** First-run or `/seed`: short survey (e.g. 3–4 questions: favorite book, favorite thing to make, one thing you’re curious about). Write answers into demo SELF (IX-B, IX-C). | Script or /seed page. |
| 6.3 | **Error handling:** Graceful message if `users/demo/` missing or malformed; empty state for empty PENDING-REVIEW. | UX. |
...
| **Data** | `users/demo/*.md` + `PENDING-REVIEW.json` | Git-friendly; matches template. |
