...
| Initiative | Deliverable | Owner |
|------------|-------------|--------|
| **Closed-loop with curriculum** | Curriculum/tutor returns outcomes (e.g. topic mastered, score) to instance; pipeline stages or links to self-evidence; Record updates via gate. Full loop: Record → curriculum → activity → evidence → Record. | Instance + integration partner. |
...
- [ANTICIPATED-APIS-3Y](anticipated-apis-3y.md) — Non-LLM APIs likely most useful over the 3-year horizon (curriculum, tutor, evidence, time, WORK, export, notifications).
