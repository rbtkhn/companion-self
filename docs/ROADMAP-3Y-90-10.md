...
| **Life skills and creation** | READ/WRITE/BUILD balance; passion projects; evidence of what they’ve made and done. | Record BUILD evidence; operator + community workshops/prompts; project templates. |
...
|------------|-------------|--------|
| **Curriculum-consumable export** | Stable schema and export (e.g. `curriculum_profile` or equivalent): IX-A, IX-B, IX-C, READ/WRITE/BUILD edge, Lexile or level, evidence anchors. Document in template; instance implements. | Template: schema doc. Instance: export script, docs. |
| **Pipeline and edge** | Pipeline produces “edge” (per skill/module) from SKILLS + EVIDENCE; edge exposed in export. No auto-merge; stage-only. | Instance. |
...
| Area | Advancement |
|------|-------------|
| **Schema** | READ/WRITE/BUILD and edge represented in export; modality (text, video, etc.) where relevant for READ. |
...
**Theme:** Record drives what to do next. Integrations with free/low-cost curriculum and tutors. Life skills and BUILD get structure. Operator effectiveness rises.
...
| **Edge-driven suggestions** | System (or operator dashboard) suggests “next at edge” for READ, WRITE, BUILD from Record. No auto-merge; suggestions only. | Instance. |
| **Life skills / BUILD templates** | Template and instance provide project prompts, workshop-in-a-box ideas, BUILD evidence templates so operators can run “afternoon” style activities without building from zero. | Template: docs. Instance: prompts, templates. |
...
| **BUILD evidence** | Schema and templates for projects, artifacts, “what they built”; link to SKILLS.BUILD dimensions. |
...
- Life skills/BUILD templates in use; BUILD evidence growing in sample Records.
...
| **Outcome metrics** | Define and track “90% value” proxies: e.g. mastery coverage (topics/levels), READ/WRITE/BUILD evidence growth, retention, operator NPS. No need to match Alpha’s SAT; do match “mastery-based, evidence-rich, portable identity.” | Template: metrics framework. Instance: instrumentation. |
...
| **Analytics** | Dashboards or reports: edge movement, evidence by READ/WRITE/BUILD, retention, cost per family. Template defines minimal set; instance implements. |
...
| **2** | Curriculum/tutor integration; edge-driven suggestions; life skills/BUILD templates; operator tools; “we did X” first-class. | Export “next best action”; integration pattern; READ/BUILD evidence quality and structure. |
...
- [INSIGHTS-ALPHA-MOONSHOTS-233-FOR-BUSINESS](INSIGHTS-ALPHA-MOONSHOTS-233-FOR-BUSINESS.md) — READ/WRITE/BUILD and business narrative.
